...
PSQL="psql --username=root --dbname=worldcup --no-align --tuples-only -c"
...
echo "$($PSQL "SELECT DISTINCT name FROM teams INNER JOIN games ON teams.team_id = games.winner_id WHERE winner_goals > opponent_goals ORDER BY name")"
...
echo "$($PSQL "SELECT name FROM games INNER JOIN teams ON games.winner_id = teams.team_id WHERE name LIKE 'Co%' ORDER BY name")"
